	...
	gmma_indices = c()
        if (length(dg) > 1 ) {
            ...
            for (gi in seq(length(dg))) {
                nodes = V(dg[[gi]])$name
		# the bipartite 'type' attribute is FALSE for variants
		n_variants = sum(! V(dg[[gi]])$type)
		# if (length(nodes) > 2) {
		if (n_variants > 1) {
		    print(sprintf("Found graph of %d nodes", length(nodes)))
...
# graph = graph_from_incidence_matrix(incidence) # up to igraph version 1.5 (at least from version 0.8)
graph = graph_from_biadjacency_matrix(incidence) # from igraph version 1.6
...
#     print(sprintf("Check for co-occuring subst in fitting group %d",gi))
    ...
    if (length(co_groups) > 0) {
        print(subst[which(! is.na(subst$gmma_reparam)),])
    }
# }
...
# ##
# ## Plot of PYR1 Diazi lib 3 graph
# ##
# # get frame with expected substitutions
# load("../data_221219/variant_counts_dia_lib13.rda")

# # assign expected library to frame with substitutions
# subst$lib = 0
# subst[which(subst$resi %in% seq(4,82)),"lib"] = 1
# subst[which(subst$resi %in% seq(97,179)),"lib"] = 3
# # zero un-expected
# subst[which(! rownames(subst) %in% expected_subst$subst),"lib"] = 0

# # assign expected library as vertex attibute
# subst_names = vertex_attr(gcp$proj2, "name")
# g = set_vertex_attr(gcp$proj2, "lib", index=subst_names, value=subst[subst_names,"lib"])

# # unexpected substitutions are poorly connected and the link between lib 1 and 3 is thin
# quartz(width=18, height=10)
# plot(g, vertex.color=vertex_attr(g,"lib"), vertex.label.cex=.7, vertex.size=5)

# # variants containing substitutions with few observations
# si = subst[which(subst$obs < 4),"i"]
# mutant[unlist(subst_mut_indices[si]),]

# # mark substitutions observed in fewer than 5 variants
# plot(g, vertex.color=subst[subst_names,"obs"]<5, vertex.label.cex=.7, vertex.size=5)
