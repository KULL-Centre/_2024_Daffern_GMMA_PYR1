...
plot(mutant$dG_init, mutant$signal+jitter, pch=".", xlim=c(-8,12),
         main=sprintf("Initial fit, dG_wt %.2f",cp$dG_wt_init), xlab="Mutant stability [kcal/mol]", ylab="Enrichment")
...
plot(mutant$dG_glob, mutant$signal+jitter, pch=".", xlim=c(-8,12), main=sprintf("Global fit, dG_wt %.2f",cp$dG_wt), xlab="Mutant stability [kcal/mol]", ylab="Enrichment")
...
for (mi in which(mutant$gmma == "hanging")) {
    ...
    print(sprintf("Added ddG = %6.2f pm %.2f (original %.2f) to subst %4d %5s based on mutant %5d %s of enrichment %.2f, dG_wt+ddG_other = %.2f, dG = %.2f and residual %.2f",
...
plot(subst$se, 1/sqrt(subst$obs), col=(subst$ddG_glob<0)+1, pch=20, main="Significant substitutions")
abline(v=max_se, h=1/sqrt(min_obs), col=8)
x=seq(0,4,.1)
# lines(x,max_see/x, col=8)
points(subst_sig$se, 1/sqrt(subst_sig$obs), col=3, pch=1)
legend("topleft", c("ddG_glob < 0","ddG_glob > 0","subst_sig","decision bounds"), col=c(2,1,3,8), pch=c(20,20,1,NA), lty=c(NA,NA,NA,1))
...
# quartz(height=6, width=6)
# plot(residue$cat, residue$cat_old)
