...
# According to graph analysis, remove varints containing the fully co-occuring bad subst
bad_subst = "M158V:F159G:A160V"
i_rm = which(grepl(bad_subst,d$var))
if (length(i_rm) > 0) {
    print(sprintf("Removing %d variants containing the fully co-occuring '%s'",length(i_rm),bad_subst))
    d = d[-i_rm,]
}

# Wild-type average log brightness, number of WT measurements and standard deviation
cns = c("seq","reads1","reads2","reads3","signal")
print(sprintf("Original column names: %s", paste(colnames(d),collapse=", ")))
print(sprintf("New column names: %s", paste(cns,collapse=", ")))
colnames(d) = cns
...
mutant = data.frame(i=seq_along(mut_list), signal=as.numeric(d[,'signal']), N_sub=sapply(mut_list, length),
                    reads1=d[,'reads1'], reads2=d[,'reads2'], reads3=d[,'reads3'])
