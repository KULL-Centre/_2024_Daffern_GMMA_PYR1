# Copyright (C) 2017-2023 Kristoffer Enoe Johansson <kristoffer.johansson@bio.ku.dk>
...
t = table(mutant[which(use_mask & mutant$active > .9),'N_sub']) #assume mutant$active is either 1 or 0
...
write.csv(dp, file="composition.csv", row.names=F, quote=F)
...
breaks = seq( floor(min(mutant[mi_use,"signal"])*10)/10 , ceiling(max(mutant[mi_use,"signal"])*10)/10, length.out=20)
plot(0,0, xlim=c(breaks[1],breaks[length(breaks)]), ylim=c(0,2), xlab="signal", ylab="density")
d$signal_mode = NA
for (nmut in d$n_mut) {
...
# if (is.na(settings$dG_wt_input)) {
#     stopifnot(is.na(cp$dG_wt))
#     init_wt = list(B_D=0.5, dG_wt=0, ddG_avg=1.7)
#     stability_wt = function(param, n_mut) {
#         e=exp((param$dG_wt + param$ddG_avg*n_mut)/settings$RT)
...
# # guess inactive baseline from highest multimutant
# init_wt$B_D = d[nrow(d),"signal_avg"]
print(sprintf("Fit WT curve with initial parameters: %s", paste(names(init_wt),"=",init_wt, collapse=", ")))
...
quartz(height=5, width=9)
plot(d$n_mut, d$signal_avg, ylab="Enrichment", ylim=c(min(mutant$signal),max(mutant$signal)), xlim=c(0,10), xlab="N_mut", col="white",
...
# manual inspection of curve fitting
verbose = FALSE
if (verbose) { par(ask=T) } else { par(ask=F) }
...
# # PYR1 binary settings
# settings$curve_max_deviance = 0.4
# settings$curve_max_stderr = 4.0
# PYR1 enrichment settings
settings$curve_max_deviance = 15.0
settings$curve_max_stderr = 1.0
...
for (si in which(subst$init_m == "none")) {
    ...
    if (length(Bavg[,1]) < 3) {
        if (verbose) { print(sprintf("SKIP %s %5d:Too few N_mutant points for fitting (%d < 3)",m,si,length(Bavg[,1]))) }
	subst[si,'init_m'] = "ce_few_Bavg"
    } else if (all(Bavg$signal_avg > cp$B_mid)) {
        if (verbose) { print(sprintf("SKIP %s %5d: Missing deactivated data for",m,si)) }
	subst[si,'init_m'] = "ce_no_inact"
    } else if (all(Bavg[seq(2,nrow(Bavg)),'signal_avg'] < cp$B_mid)) { 
        if (verbose) { print(sprintf("SKIP %s %5d: Missing N_mutants that are active (on average). N of first N-mut: %d", m, si, Bavg[2,'n_mut'])) }
	...
	if (verbose) {
            ...
            plot(Bavg$n_mut, Bavg$signal_avg, ylab="Enrichment", ylim=c(min(c(cp$B_D,mutant$signal)),max(c(cp$B_max,mutant$signal))), xlim=c(0,10), xlab="N_mut", main=s)
            abline(h=cp$B_mid, col="lightgray")
            text(Bavg$n_mut, Bavg$signal_avg, labels=Bavg$n_data, pos=1)
            x = c(0,seq(1,10,.1))
            # when comparing to WT, compare wt+tm to wt such that wt+tm at x=1 is compared to wt at x=1. Don't shift tm curve so it still match the points
            lines(x+1, stability_wt(fit_wt$par, x), col=2, lwd=2)
            lines(x, stability(list(ddG_tm=subst[si,'init_ddG'], ddG_other=coef(fit)['ddG_other']), x))
	    # quartz.save("fit_subst.png", type="png")
	}
    }
    if (verbose) {
        if (! subst[si,'init_m'] %in% c("1_curve","ce_fit_info","ce_high_dev")) {
            plot(Bavg$n_mut, Bavg$signal_avg, ylab="Enrichment", ylim=c(min(c(cp$B_D,mutant$signal)),max(c(cp$B_max,mutant$signal))), xlim=c(0,10), xlab="N_mut",
...
for (pass in seq(iterations)) {
    ...
    print(table(subst$init_m))
    if (est == prev_est & pass>2) { print("No more estimation are being made, break"); break } else { prev_est = est}
    print("")
}
